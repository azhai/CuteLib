...
defined('CUTELIB_FILE') or define('CUTELIB_FILE', false);


//将框架打包为单个文件
if (CUTELIB_FILE) {
    //Importer.php文件压缩后，大小在1.5KB以上
    if (! is_readable(CUTELIB_FILE) || filesize(CUTELIB_FILE) < 1024) {
        ...
        $bale->minify(CUTELIB_FILE, $files,
                glob(SRC_ROOT . '/Cute/*/*.php'));
    }
    require_once CUTELIB_FILE; // 使用压缩后的文件
} else {
